...
(defn my-range [up-to]
  (let [next-val (dec up-to)]
    (if (== up-to 0)
      ()
      (cons next-val (my-range next-val)))))
...
(defn inits [a-seq]
  (let [reversed-a-seq (reverse a-seq)
        all-but-last (fn [another-seq]
                       (reverse (rest (seq reversed-a-seq))))]
    (if (empty? a-seq)
      (cons () (seq a-seq))
      (cons (seq a-seq) (inits (all-but-last a-seq))))))
